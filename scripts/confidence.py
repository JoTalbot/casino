...
def main() -> int:
    ...
    # Порог сходимости обязан зависеть от волатильности, а не быть
    # константой. Ошибка среднего это sigma / sqrt(n): при переходе на
    # бонусную математику sigma выросла с 4.1 до 11.5, и прежние 0.2 п.п.
    # стали недостижимы физически — проверка начала «падать» на здоровой
    # выборке. Берём тройную ошибку среднего, но не мягче 0.2 п.п. (T-211).
    spins_used = report.sample_rounds
    limit_pp = max(0.2, 3 * report.sigma / (spins_used ** 0.5) * 100)
    checks.append((
        f"Выборка сходится к аналитике (<= {limit_pp:.2f} п.п. = 3 sigma/sqrt(n))",
        dev <= limit_pp,
        f"{dev:.4f} п.п. при sigma {report.sigma:.2f} и n {human(spins_used)}",
    ))
