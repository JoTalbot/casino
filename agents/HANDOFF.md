# HANDOFF.md — Передача смены

**От кого:** `arena-2026-08-20-T`
**Кому:** следующему агенту
**Дата:** 2026-08-20

## Первое, что нужно знать

Смены H…Q (T-104…T-173) закрывались формально: лок, JOURNAL, TASKS, push —
и ни строчки кода. Проверил через `git log --stat`. Не доверяй строкам
«Final polish» в TASKS.md за этот период; проверяй, что реально в дереве.

Чтобы это не повторилось, `scripts/check_protocol.py` теперь предупреждает,
если шесть и более коммитов подряд трогают только `agents/**` (ADR-021).

## Что сделано в смену R

Реальные баги, найденные чтением кода, а не бэклога:

1. **`/api/v1/referrals/progress` → 500, `/referrals/leaderboard` → 404 (T-177).**
   В `app.ts` регистрация второго маршрута оказалась *внутри* тела обработчика
   первого. Fastify не даёт добавлять роуты после старта, поэтому progress падал,
   а leaderboard не существовал вовсе. Клиент (`ref.html`, `main.ts`) дёргает оба.
2. **Накрутка рефералов (T-180).** `createReferral` проверял только «реферал ещё
   не привязан». Любой игрок мог начислить себе 5000 CHIP, подставив чужой UUID.
   Теперь: оба игрока должны существовать, приглашённый — без сыгранных раундов.
3. **Ачивки (T-179).** Открытие ачивки и начисление награды шли отдельными
   запросами вне транзакции, с бессмысленным `FOR UPDATE`. Свёл в одну
   транзакцию; при гонке награда выдаётся один раз.
4. **Push (T-178).** Подписки жили в `Map` внутри процесса — терялись при
   рестарте и не работали на втором инстансе. Перенёс в таблицу
   `push_subscriptions` (миграция `db/migrations/0004_push.sql`),
   добавил `DELETE /api/v1/push/subscribe`.
5. **Чат (T-176).** Длина проверялась до `trim`. Плюс чистка Map счётчиков.
6. **Тесты (T-175).** `src/server/fakeDb.ts` — стаб `Database`. 19 новых тестов
   на маршруты соц-слоя, которые идут в CI без PostgreSQL (ADR-020).
7. **Документация (T-181).** `docs/API.md` §3.8 — все соц-эндпоинты, коды ошибок.

Проверки на конец смены: `npm run typecheck` — чисто, `npm test` — 136 pass /
0 fail / 3 skip (PG), `npm run client:test` — 21 pass, `check_protocol.py` — ok.

## Что взять следующим — T-182 (P1)

**Поднять PostgreSQL сервисом в CI.** Сейчас `full.integration.test.ts`,
`admin.integration.test.ts` и `migrate.integration.test.ts` пропускаются,
потому что нет `TEST_DATABASE_URL`. Именно в этой слепой зоне и жили баги
T-177/T-180. В `.github/workflows/ci.yml` в задачу `engine` добавить
`services: postgres:16` и переменную `TEST_DATABASE_URL`, затем прогнать
`node build/server/migrate.js`. Лок: `infra`.

Заодно проверь миграцию `0004_push.sql` на живой базе — она ещё не
применялась нигде, кроме теории.

## Боевой стенд (добавлено после деплоя)

Развёрнут на сервере владельца: `/root/casino`, `docker compose -p casino`
с оверлеем `deploy/docker-compose.shared-host.yml`. Витрина наружу —
`http://167.233.95.7:21899`, API `127.0.0.1:21110`, PostgreSQL без публикации.
Сервер общий: nginx с чужими сайтами, девять aios-контейнеров — не трогать.
Домен `api.autosklo.org.ua` занят чужим сайтом, казино там не вешать.

Обновление стенда:
```bash
cd /root/casino && git pull origin main
docker compose -p casino -f docker-compose.yml \
  -f deploy/docker-compose.shared-host.yml up -d --build
```

Деплой вскрыл четыре бага, невидимых для CI (T-184…T-187) — все закрыты.
Это лучший аргумент за T-182.

## Клиент: графика и производительность (T-190, T-191)

Автомат переделан: символы и корпус нарисованы кодом и **запекаются в
текстуры** при старте (`initSymbolTextures`, `bake` в `cabinet.ts`).
Правило: во время игры сцена ничего не рисует заново — меняются только
transform, alpha и текстура спрайта. Если добавляете эффект, добавляйте
его так же.

Две ловушки Pixi v8, на которые уже наступили (обе — «объект в графе есть,
пикселей нет»):

1. `visible = false` до первого кадра сцены — объект больше не вернуть
   одним `visible = true`. Добавляйте и удаляйте из родителя.
2. `Text` с пустой строкой, наполненный позже, не рисуется. Создавайте
   текст сразу с содержимым.

Замеры снимались через `?debug=1` → `window.__casino`.

## Telegram Mini App и бонус-игра (T-195…T-197)

Мини-апп: `https://api.autosklo.org.ua/parts/casino/`. Путь странный по
причине: на домене Cloudflare Access, наружу пропускается только `/parts/*`
(ADR-023). Локации в чужом nginx добавлены аккуратно, бэкап конфига лежит в
`/root/nginx-api.autosklo.backup-*.conf`. Клиент собирается с
`BASE_PATH=/parts/casino/`, обращения к API строятся от `BASE_URL`.

Бонус-игра меняет только форму серии фриспинов, не количество денег
(ADR-022). Если будете трогать множители — обязательно прогоните
`node scripts/bonus_ev.mjs 6000000` и сверьте RTP тиров: две ловушки
(scatter и ретриггер) уже стоили 0.34 п.п.

Секреты в `/root/casino/.env`: токен бота, секрет вебхука, адрес мини-аппа.
В git их нет.

## Смотрите на CI после пуша

За смену R накопилось четыре красных прогона, которые никто не открыл.
Падение было плавающим и указывало не туда: виноват был не тест админки,
а параллельный прогон двух интеграционных тестов над одной базой (T-201).
Проверка занимает секунды:

```bash
curl -s -H "Authorization: token <PAT>" \
  "https://api.github.com/repos/JoTalbot/casino/actions/runs?per_page=5" \
  | python3 -c "import sys,json;[print(r['conclusion'], r['head_commit']['message'][:50]) for r in json.load(sys.stdin)['workflow_runs']]"
```

## Известные проблемы

- Фильтр чата переписан (T-206), но транслит вроде «xyu» не ловит — это
  зафиксировано тестом как осознанный компромисс. Для продакшена нужен
  отдельный сервис модерации.
- `sendPushToPlayer` — заглушка, реальной доставки нет (нужны VAPID-ключи).
- PAT из чата владельца всё ещё не отозван — Q-006 открыт.

## Перед стартом

```bash
git pull --rebase origin main
cat AGENTS.md agents/STATE.md agents/HANDOFF.md
tail -60 agents/JOURNAL.md
ls agents/locks/
npm install && npm test
```
