...
**ID не переиспользуются.** Следующий свободный ID: **T-042**

---

## Активные и запланированные

**Весь бэклог T-001…T-041 закрыт.** Новых активных задач нет.
...
Ранее закрытые (T-001…T-037 — done):
...
- **Блокировок больше нет.** Q-001 закрыт решением ADR-004 (вариант A — соц-казино на виртуальных фишках), T-010, T-011, T-014, T-016 разблокированы.
- **T-016** переформулирована: юр. трек теперь про соц-казино, а не про выбор игорной лицензии. Лицензионный трек вернётся, если владелец выберет B или C (тогда ADR-004 меняет статус, см. QUESTIONS.md, Q-001).
- **T-019 … T-022** порождены разбором OSS (T-009), см. `research/06-OSS-REVIEW.md` §6. Все четыре закрыты. T-019 дал ADR-007 (коридор приёмки RTP зависит от объёма выборки) и числа для T-015: депозит в 50 ставок заканчивается в 65% сессий.
- **Q-006** открыт по итогам T-016 и помечен 🔴: судебная практика 2025–2026 (High 5 Games $24.9 млн, иск генпрокурора Вашингтона на $225+ млн) показала, что безопасность варианта A держится на отсутствии покупок фишек. До ответа владельца монетизация не реализуется, см. `research/07-SOCIAL-CASINO-LEGAL.md`.
- **Q-002 … Q-005** остаются открытыми, но ничего не блокируют: они влияют на стадию 2 (рынки, бюджет, платежи, агрегатор), а не на разработку ядра.
