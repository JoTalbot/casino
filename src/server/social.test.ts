...
test("фильтр чата: ловит обходы и не трогает обычный текст (T-206)", async () => {
  const { chatFilter } = await import("./chat.js");
  const blocked = ["это scam", "с к а м", "s.c.a.m", "сссукаа", "иди на хуй", "блядский автомат", "спам рассылка"];
  for (const text of blocked) {
    assert.equal(chatFilter.containsBadWord(text), true, `должно блокироваться: ${text}`);
  }
  // Ложные срабатывания дороже пропусков: игрок не понимает, за что его
  // отклонили. «всем удачи» содержит «муда», «скамейка» — «скам».
  const allowed = ["всем удачи за столом", "скамейка в парке", "какой хороший спин", "мудрое решение", "спасибо за игру"];
  for (const text of allowed) {
    assert.equal(chatFilter.containsBadWord(text), false, `не должно блокироваться: ${text}`);
  }

  // Чего фильтр НЕ ловит и не претендует: транслит вроде «xyu» и
  // изобретательные замены. Это компромисс — гнаться за всеми вариантами
  // значит начать блокировать нормальную речь. Для продакшена нужен
  // отдельный сервис модерации.
  assert.equal(chatFilter.containsBadWord("xyu"), false, "известное ограничение, зафиксировано намеренно");
});
